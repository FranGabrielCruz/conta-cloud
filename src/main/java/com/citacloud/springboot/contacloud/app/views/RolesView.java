...
public class RolesView extends VerticalLayout implements BeforeEnterObserver {
    private static final List<String> ORDEN_SECCIONES = List.of("Empresa", "Usuarios", "Roles y permisos", "Sucursales",
        "Monedas", "Tasas de cambio", "Impuestos", "Comprobantes fiscales", "Secuencias",
    ...
    private static Div crearGrupoPermisos(String titulo,List<OpcionPermiso> opciones,RolDto actual,Map<UUID,Checkbox> checks){var grupo=new Div();grupo.addClassName("cc-permission-group");grupo.add(new H3(titulo));for(OpcionPermiso opcion:opciones){PermisoDto p=opcion.permiso();var check=new Checkbox(p.nombre());check.setHelperText(p.descripcion());check.setValue(actual!=null&&opcion.idsEquivalentes().stream().anyMatch(actual.permisoIds()::contains));checks.put(p.id(),check);grupo.add(check);}return grupo;}
    private static String seccionPermiso(PermisoDto p){String recurso=valor(p.recurso()).toLowerCase(Locale.ROOT);String codigo=p.codigo().toUpperCase(Locale.ROOT);if(recurso.isBlank()){if(codigo.startsWith("CONFIGURACION_CONTABLE"))recurso="configuracion_contable";else if(codigo.startsWith("CONDICION_PAGO"))recurso="condicion_pago";else if(codigo.startsWith("TASA_CAMBIO"))recurso="tasa_cambio";else recurso=codigo.contains("_")?codigo.substring(0,codigo.indexOf('_')).toLowerCase(Locale.ROOT):codigo.toLowerCase(Locale.ROOT);}return switch(recurso){case"empresa"->"Empresa";case"usuario","usuarios"->"Usuarios";case"rol","roles"->"Roles y permisos";case"sucursal","sucursales"->"Sucursales";case"moneda","monedas"->"Monedas";case"tasa_cambio","tasas_cambio"->"Tasas de cambio";case"impuesto","impuestos"->"Impuestos";case"comprobante","comprobantes"->"Comprobantes fiscales";case"secuencia","secuencias"->"Secuencias";case"condicion_pago","condiciones_pago"->"Condiciones de pago";case"periodo","periodos"->"Períodos fiscales";case"configuracion_contable"->"Configuración contable";case"auditoria"->"Auditoría";default->"General";};}
