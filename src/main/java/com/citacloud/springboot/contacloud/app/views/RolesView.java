...
public class RolesView extends VerticalLayout implements BeforeEnterObserver {
    ...
    private List<OpcionPermiso> opcionesPermisos(){
        Map<String,List<PermisoDto>> equivalentes=service.catalogoPermisos().stream().collect(Collectors.groupingBy(RolesView::claveEquivalencia,LinkedHashMap::new,Collectors.toList()));
    ...
    static String claveEquivalencia(PermisoDto p){String seccion=seccionPermiso(p);String recurso="Configuración".equals(seccion)?subseccionConfiguracion(p):seccion;return seccion+"|"+recurso+"|"+accionPermiso(p);}
